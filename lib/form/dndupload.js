    ...
    Y.extend(dnduploader, Y.Base, {
        ...
        initialise_queue: function(files) {
            ...
            for (i=0; i<files.length; i++) {
                ...
                var allowed_type = this.options.acceptedtypes;

                // Check file type.
                if (!(allowed_type.length === 0 || (allowed_type.length === 1 && allowed_type[0] === "*"))) {
                    if (allowed_type.indexOf(extension) === -1) {
                        this.print_msg(M.util.get_string('invalidfiletype', 'error', files[i].name), 'error');
                        this.uploadqueue = []; // No uploads if one file is wrong type.
                        return;
                    }
                }
